SCRIPT_TITLE = "Microtonal - Nearest EDO Note"
...
function main()
    add_nearest_edo_pitch_deviation()
    SV:finish()
end

-- Get the cent value above A of the SynthV note's pitch.
function get_cents_of_synthv(synthv_number)
    return ((synthv_number + 3)*100)%1200
end
...
-- Get the cent difference of the nearest note in EDO to the original SynthV note.
function get_nearest_edo_cent_difference(edo_number, synthv_note)
    local closest_cent_difference = get_cents_of_edo(edo_number, 0) - get_cents_of_synthv(synthv_note)
    for i = 0, edo_number - 1 do
        local edo_cent_difference = get_cents_of_edo(edo_number, i) - get_cents_of_synthv(synthv_note)
        if math.abs(edo_cent_difference) < math.abs(closest_cent_difference) then
            closest_cent_difference = edo_cent_difference
        end
    end
...
function add_nearest_edo_pitch_deviation()
